...
function readSlugs(value: string | null) {
  return [
    ...new Set((value ?? "").split(",").map((slug) => slug.trim())),
  ].filter(Boolean);
}
...
function CompareSelectionBar({
  ...
  const available = mountains.filter(
    (mountain) => !selectedSlugs.includes(mountain.slug),
  );
      ...
      <CardContent className="flex flex-col gap-md p-md md:flex-row md:items-center md:justify-between">
        ...
        <div className="flex flex-wrap items-center gap-xs">
          ...
          <select
            ...
            disabled={
              selectedSlugs.length >= MAX_COMPARE_ITEMS || !available.length
            }
            id="compare-add"
            onChange={(event) => {
...
function getBestIndexes(mountains: CompareMountain[], row: Row) {
  ...
  return new Set(
    values.flatMap((value, index) => (value === best ? [index] : [])),
  );
}
...
function ComparisonTable({ mountains }: { mountains: CompareMountain[] }) {
  const rows: Row[] = [
    ...
    { key: "name", label: "Nama", value: (mountain) => mountain.name },
    {
      key: "province",
      label: "Provinsi",
      value: (mountain) => mountain.province,
    },
    {
      key: "elevation",
      label: "Elevasi",
      value: (mountain) => `${mountain.elevation.toLocaleString("id-ID")} mdpl`,
      score: (mountain) => mountain.elevation,
      rank: "max",
    },
    {
      key: "difficulty",
      label: "Kesulitan",
      value: (mountain) => DIFFICULTY_LABELS[mountain.difficulty],
    },
    {
      key: "duration",
      label: "Durasi",
      value: (mountain) => `${mountain.durationDays} hari`,
      score: (mountain) => mountain.durationDays,
      rank: "min",
    },
    {
      key: "beginner",
      label: "Beginner friendly",
      value: (mountain) => `${mountain.beginnerScore}/100`,
      score: (mountain) => mountain.beginnerScore,
      rank: "max",
    },
    {
      key: "camping",
      label: "Camping",
      value: (mountain) => formatBoolean(mountain.campingAvailable),
      rank: "true",
    },
    {
      key: "water",
      label: "Sumber air",
      value: (mountain) => formatBoolean(mountain.waterSource),
      rank: "true",
    },
    {
      key: "season",
      label: "Musim terbaik",
      value: (mountain) => mountain.bestSeason,
    },
    {
      key: "popularity",
      label: "Popularitas",
      value: (mountain) => `${mountain.popularityScore}/100`,
      score: (mountain) => mountain.popularityScore,
      rank: "max",
    },
    {
      key: "sunrise",
      label: "Rating sunrise",
      value: (mountain) => formatRating(mountain.sunriseRating),
      score: (mountain) => mountain.sunriseRating,
      rank: "max",
    },
  ];
  ...
  return (
    <div
      className="overflow-x-auto rounded-xl border border-divider"
      role="region"
      aria-label="Tabel perbandingan gunung"
      tabIndex={0}
    >
      <table className="w-full min-w-[48rem] border-collapse text-left text-body-sm">
        <caption className="sr-only">
          Perbandingan atribut gunung yang dipilih
        </caption>
        <thead className="sticky top-0 z-10 bg-card">
          <tr className="border-b border-divider">
            <th
              className="sticky left-0 z-20 w-40 bg-card p-sm font-semibold text-text-secondary"
              scope="col"
            >
              Atribut
            </th>
            {mountains.map((mountain) => (
              <th
                className="min-w-48 p-sm align-top font-semibold text-text-primary"
                key={mountain.slug}
                scope="col"
              >
                {mountain.name}
        ...
        <tbody>
          {rows.map((row) => {
            ...
            return (
              ...
              >
                <th
                  className="sticky left-0 z-[1] bg-card p-sm font-medium text-text-secondary"
                  scope="row"
                >
                  {row.label}
                </th>
                {mountains.map((mountain, index) => {
                  ...
                  return (
                    <td
                      className={cn(
                        "p-sm align-middle text-text-primary",
        ...
        <Card key={mountain.slug}>
          ...
          <CardContent className="flex flex-wrap gap-xs">
            <Link
              ...
              onClick={() =>
                trackEvent("detail_click", { mountain: mountain.slug })
              }
            >
              Lihat detail
            </Link>
...
function CompareAiSummary({
  ...
  if (isLoading) {
    return (
      <Card aria-label="Memuat ringkasan AI">
      ...
      <Card className="border-dashed">
        <CardContent className="p-md">
          ...
          <p className="mt-2xs text-body-sm text-text-secondary">
            Perbandingan data di bawah tetap dapat digunakan. Ringkasan AI akan
            muncul saat layanan tersedia.
          </p>
        </CardContent>
      </Card>
    );
  ...
  return (
    <Card className="border-primary/20 bg-primary/5">
      <CardHeader>
        <CardTitle className="text-h3">Ringkasan perbandingan</CardTitle>
      </CardHeader>
      <CardContent className="space-y-md text-body-sm text-text-secondary">
        ...
        <div className="grid gap-md md:grid-cols-2">
          <div>
            <h3 className="font-semibold text-text-primary">Perbedaan utama</h3>
            <ul className="mt-xs list-disc space-y-2xs pl-md">
        ...
        <div>
          <h3 className="font-semibold text-text-primary">
            Kelebihan berdasarkan data
          </h3>
          <ul className="mt-xs grid gap-xs md:grid-cols-2">
            {summary.strengths.map((item) => (
              <li
                className="rounded-md border border-divider bg-background p-xs"
                key={item.mountainId}
              >
                <span className="font-medium text-text-primary">
                  {item.mountainName}
        ...
        <p className="border-t border-divider pt-sm text-text-primary">
          {summary.cta}
        </p>
      </CardContent>
    </Card>
...
export function ComparePage({ mountains }: ComparePageProps) {
  ...
  useEffect(() => {
    ...
    fetch("/api/compare/summary", {
      ...
      body: JSON.stringify({
        slugs: selectedMountains.map((mountain) => mountain.slug),
      }),
      signal: controller.signal,
    })
      .then(async (response) =>
        response.ok
          ? ((await response.json()) as { summary?: CompareSummary | null })
          : { summary: null },
      )
      .then((result) => {
        if (!controller.signal.aborted) {
          ...
          if (result.summary)
            trackEvent("ai_summary_view", { count: selectedMountains.length });
        }
      })
      .catch(() => {
  ...
  const updateSelection = (next: string[]) => {
    ...
    router.replace(
      query ? `/compare?mountains=${encodeURIComponent(query)}` : "/compare",
      { scroll: false },
    );
  };
      ...
      <Container className="space-y-xl">
        <header className="max-w-3xl">
          <div className="flex items-center gap-xs text-primary">
            ...
            <p className="text-label font-semibold">Bandingkan gunung</p>
          </div>
          <h1 className="mt-xs font-heading text-h1 font-semibold text-text-primary">
            Pilih puncak yang paling cocok untuk langkahmu.
          </h1>
          <p className="mt-sm max-w-reading text-body-lg text-text-secondary">
            Bandingkan data penting secara berdampingan, lalu lanjutkan ke
            detail atau Jejak Finder untuk keputusan yang lebih personal.
          </p>
        </header>
        ...
        <CompareSelectionBar
          ...
          onClear={() => {
            updateSelection([]);
            trackEvent("mountain_removed", { mountain: "all" });
          }}
          onRemove={(slug) => {
            updateSelection(selectedSlugs.filter((item) => item !== slug));
            trackEvent("mountain_removed", { mountain: slug });
          }}
        ...
        {selectedMountains.length >= 2 ? (
          <section aria-labelledby="comparison-heading" className="space-y-md">
            <div>
              ...
              <p className="mt-2xs text-body-sm text-text-secondary">
                Sel yang diberi penanda menunjukkan nilai paling menonjol pada
                baris tersebut.
              </p>
            </div>
          ...
          <Empty className="min-h-72 border-divider bg-surface">
            <EmptyHeader>
              ...
              <EmptyDescription>
                Gunakan pilihan di atas untuk menambahkan gunung dan mulai
                membandingkan atributnya.
              </EmptyDescription>
            </EmptyHeader>
            <EmptyContent>
              <Link
