...
export function MountainMap({ mountains }: MountainMapProps) {
  ...
  return (
    <div className="space-y-md">
      ...
      <section aria-labelledby="map-list-heading" className="space-y-sm lg:hidden"><h2 className="flex items-center gap-xs font-heading text-h4 font-semibold text-text-primary" id="map-list-heading"><List aria-hidden="true" className="size-4 text-primary" />Daftar gunung</h2><ol className="grid gap-xs sm:grid-cols-2">{visibleMountains.map((mountain) => <li key={mountain.slug}><button aria-current={selectedSlug === mountain.slug ? "true" : undefined} className={cn("w-full rounded-md border border-divider bg-surface p-sm text-left text-body-sm", selectedSlug === mountain.slug && "border-primary bg-primary/5")} onClick={() => { setSelectedSlug(mountain.slug); setMobilePreviewOpen(true); trackEvent("preview_open", { mountain: mountain.slug }); }} type="button"><span className="font-medium text-text-primary">{mountain.name}</span><span className="mt-2xs block text-caption text-text-secondary">{mountain.province} · {DIFFICULTY_LABELS[mountain.difficulty]}</span></button></li>)}</ol></section>
