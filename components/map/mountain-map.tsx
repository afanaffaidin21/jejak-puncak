...
function uniqueValues(mountains: MapMountain[], key: "island" | "province") {
  return [...new Set(mountains.map((mountain) => mountain[key]))].sort((a, b) =>
    a.localeCompare(b),
  );
}
    ...
    <Card className="overflow-hidden">
      ...
      <CardContent className="space-y-sm">
        ...
        <div className="flex flex-wrap gap-xs text-caption text-text-secondary">
          <span>{mountain.elevation.toLocaleString("id-ID")} mdpl</span>
          <span>·</span>
...
export function MountainMap({ mountains }: MountainMapProps) {
  ...
  const allBoundsRef = useRef<[[number, number], [number, number]] | null>(
    null,
  );
  ...
  const provinces = useMemo(
    () => uniqueValues(mountains, "province"),
    [mountains],
  );
  ...
  const filterClass =
    "h-10 w-full rounded-md border border-divider bg-background px-sm text-body-sm text-text-primary outline-none focus-visible:ring-2 focus-visible:ring-primary";
    ...
    <div className="space-y-md">
      ...
      {!token ? (
        <Empty className="min-h-96 border-divider bg-surface">
          <EmptyHeader>
            ...
            <EmptyTitle>Peta interaktif memerlukan konfigurasi</EmptyTitle>
            <EmptyDescription>
              Tambahkan <code>NEXT_PUBLIC_MAPBOX_TOKEN</code> (token publik
              Mapbox) di environment untuk mengaktifkan peta.
            </EmptyDescription>
          </EmptyHeader>
        </Empty>
      ) : (
      ...
      >
        <SheetContent className="p-0" side="bottom">
          <SheetHeader>
            <SheetTitle>Preview gunung</SheetTitle>
            <SheetDescription>
              Ringkasan singkat dari marker yang dipilih.
            </SheetDescription>
          </SheetHeader>
          {selectedMountain ? (
            <div className="p-md">
